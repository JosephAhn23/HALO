...
_SCIENTIFIC_HINT = re.compile(
    r"\$|\\frac|\\int|\btheorem\b|\blemma\b|\bproof\b|\bcorollary\b|"
    r"\bfigure\s*\d|\bplot\b|\bmatplotlib\b|\bnumpy\b|\bscipy\b|"
    r"\bdataset\b|\bhypothesis\b|\bregression\b",
    re.IGNORECASE,
)
_HAND_WAVING = re.compile(
    r"\b(clearly|obviously|trivially|it is easy to (see|show)|without loss of generality)\b",
    re.IGNORECASE,
)
_REWARD_HACK_HINTS = [
...
def scan_scientific_integrity(answer: str) -> dict[str, Any]:
    ...
    for rx, label in _REWARD_HACK_HINTS:
        if rx.search(text):
            flags.append(label)
    deep_reason = len(flags) >= 2 or "hardcoded_plot_coordinates" in flags
    return {
        "scientific_context": bool(_SCIENTIFIC_HINT.search(text)),
        "integrity_flags": flags,
        "deep_reason_recommended": deep_reason,
    }
...
class VerifierAgent(BaseAgent):
    ...
    def process(self, task: AgentTask) -> AgentResult:
        ...
        if integrity.get("deep_reason_recommended"):
            meta["deep_reason_mode"] = (
                "Re-derive key results from first principles with explicit assumptions; "
                "avoid hard-coded plot data and unmarked synthetic RNG."
            )
